...
const productsFiltered = products1.filter((product) => {
    if (product.photos !== undefined && product['photos'].length > 0) return true;
    else return false;
});
...
for (let x = 0; x < en.length; x++) {
    daysOfWeek[en[x]];
    for (let y = 0; y < ru.length; y++) {
        if (x === y) {
            daysOfWeek[en[x]] = ru[y];
        }
    }
}
